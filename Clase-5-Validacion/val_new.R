...
archivos <- list.files(pattern = paste0(".*",var,".*historical.*\\.nc$"))
archivos
...
for (j in 1:length(archivos)){
  ...
  for (i in 1:length(files)){
    ...
    tail(data)
    length(data$pp)
    model <- ReadNetCDF(archivos[j],subset=list(lat=meta$LATITUDE[i],lon=meta$LONGITUDE[i]+360),vars=var)
    model$time <- as.Date(model$time)
    model <- model[model$time >= as.Date("1981-01-01") & model$time <= as.Date("1995-12-31"),]
    colnames(model)[1] = 'date'
    model = model %>% inner_join(data, by= 'date' )
   
    if (var== "pr"){
      data <- data.frame(Date=model$date,var_obs=model$pp,var_model= model$pr)
    }else if (var== "tasmax"){
      data <- data.frame(Date=model$date,var_obs=model$tmax,var_model= model$tasmax-273.15)
    }else{
      data <- data.frame(Date=model$date,var_obs=model$tmin,var_model= model$tasmin-273.15)
    }
...
for (j in 1:length(archivos)){
  ...
  for (i in 1:length(files)){
    ...
    if (var== "pr"){
      ...
      data <- data.frame(Date=data$date,var_obs=data$tmax,var_model= model$tasmax-273.15)
    }else{
      model <- model %>% group_by(Year,Month) %>%
        summarise(tasmin=mean(tasmin))
      
      
      data <- data.frame(Date=data$date,var_obs=data$tmin,var_model= model$tasmin-273.15)
    }
