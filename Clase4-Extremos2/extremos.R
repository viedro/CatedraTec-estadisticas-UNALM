...
setwd("D:/Catedra_2024-2/Clase4-Extremos")
getwd()


##Generando fichero por estacion

archivo <- substr(list.files(pattern = "^ho000.*\\.csv$"),1,10)

estacion<- list()
for (i in 1:length(archivo)){
  estacion[[i]] <- read.csv(paste0(archivo[[i]],".csv"))
  #Exportando fichero
...
ci <- climdexInput.raw(data$Tmax,
                       data$Tmin, data$Pp,
                       Fecha,Fecha,Fecha, base.range=c(1980, 1995),
...
funciones <- list("climdex.dtr","climdex.cdd","climdex.csdi","climdex.dtr" ,"climdex.prcptot", "climdex.cwd", "climdex.r10mm","climdex.r95ptot","climdex.fd","climdex.gsl","climdex.id","climdex.r20mm")
...
for (j in 1:length(funciones)){
  ...
  for (i in 1:length(files)){ 
    ...
    ci <- climdexInput.raw(as.numeric(estacion[[i]]$Tmax),
                           ...
                           Fecha_pcic,Fecha_pcic,Fecha_pcic, base.range=c(1980, 1995),
                           northern.hemisphere = F)
    ...
    if (i==length(estacion)){
      ...
      if (nrow(index_df2 > 55)){
        ...
        graph <- ggplot(na.omit(melt(index_df2, id.vars = "Fecha",variable.name = "Estacion")), aes(x = Fecha, y = value)) +
